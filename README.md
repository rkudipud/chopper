...
- **`reference_file` extended to `add_step_before` / `add_step_after` (FR-55 revised).** The mechanism 4.7.0 shipped for whole-stage `steps` now also works on the block of `items` a feature injects at a `reference` anchor: set `reference_file` (domain-relative path) instead of authoring `items` inline. Same P1 resolution, same UTF-8/BOM-tolerant decoding, same `\r\n`/`\r`/`\n` line-splitting, same verbatim blank/comment-line preservation. Mutually exclusive with `items` per action (schema `oneOf`; both or neither fails the existing generic `VE-02`/`VE-12`). `VE-39`'s slug broadened from `stage-reference-file-invalid` to `reference-file-invalid` since it is no longer stage-scoped -- same code number, no new registry slot consumed. `replace_step` deliberately does not get `reference_file`: its `with` is exactly one replacement string, not a block. Step-level `reference_file` is provenance only up to P1 load (`AddStepAction.reference_file`); unlike stage-level `reference_file`, it is not retained past P3 flow resolution once the block is spliced into the target stage. See ARCHITECTURE.md Sec.3.6, Sec.6.7, FR-55.

### 4.7.0 -- 2026-09-14

- **New: stage steps sourced from a reference file (`reference_file`, issue #28).** A stage definition (base `stages[]`, or a feature `add_stage_before` / `add_stage_after` / `replace_stage`) may now set `reference_file` (a domain-relative path) instead of authoring `steps` inline. Chopper reads the file in P1, decodes UTF-8 tolerating a leading BOM, and splits it into one step per physical line -- any of `\r\n` / `\r` / `\n` line endings, blank and comment lines preserved verbatim, no per-line trimming. Mutually exclusive with `steps` per stage (schema `oneOf`; both or neither fails with the existing `VE-02`/`VE-12`, no new code needed for that case). New `VE-39 stage-reference-file-invalid` covers a missing, unreadable, undecodable, or empty-after-split file. New advisory `VW-26 stage-reference-file-not-preserved` (gated by `options.cross_validate`, alongside VW-14/15/16) fires when the referenced file is not otherwise kept via `files.include` -- R1 default-exclude still applies to it. A Case 2 re-trim reads the file from `<domain>_backup/`, matching every other P1 domain-relative read. `StageDefinition.reference_file` / `StageSpec.reference_file` retain the original path as provenance only; resolved `steps` are indistinguishable downstream from inline authoring. See ARCHITECTURE.md Sec.3.6, FR-55.
...
- **Documentation refresh.** All technical and user docs updated to match actual v3.4 implementation: ENGINEERING.md Sec.3 module layout rewritten with all 14 packages and files; tool-command pool descriptions updated to 6 bundled vendor pools (PrimeTime, PrimePower, PrimeECO, PrimeSim, Formality, PrimeClosure); user_docs live-trim steps, module map, FAQ, and flag descriptions updated. Version bumped 3.4.0 -> 3.4.1.
...
- **Gate status:** ruff lint + format [OK], mypy [OK] (78 source files), `lint-imports` [OK] (4/4 contracts kept, 0 broken), pytest **1097 passed / 10 skipped**, coverage **94.34%** (gate 78%). No diagnostic-registry, schema, CLI, or audit-bundle field-shape changes (additive `files_skipped_decode` field only).
