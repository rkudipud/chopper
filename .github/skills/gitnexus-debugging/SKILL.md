...
> Availability protocol: use this workflow only when GitNexus MCP tools or `gitnexus://...` resources are exposed in the current session. If MCP is unavailable, read the relevant `.github/agent_memory/*.md` file first, then debug with `search/textSearch`, `search/codebase`, `read/readFile`, and focused tests.
