...
- **2026-05-08 R1 ordered-overlay rewrite COMPLETE — version 2.0.0-alpha.** Replaced the additive-only feature model with an ordered overlay (last-layer-wins per file/proc). Architecture doc, ENGINEERING, IMPLEMENTATION, DIAGNOSTIC_CODES, CLI_REFERENCE, JSON_AUTHORING_GUIDE, schemas, and all subordinate docs cascaded. Diagnostic registry: `VW-18 cross-source-pe-vetoed` and `VW-19 cross-source-fe-vetoed` retired (slots preserved per never-reuse policy); new `VW-21 layer-shadowed` (warning, exit 0) and `VE-27 no-op-exclude` (error, exit 1) added. `VE-27` registered but emission not yet implemented (validator has no check yet; merge_service defers). `ShadowEvent(layer, prior_layer, action)` added; `FileProvenance` now carries `contributed_by` + `shadowed_by[]`. Layer-key format: `"base"`, `"feature:<name>"`, with optional sub-tags like `":files.include"` / `":procedures.include"` / `":flow_actions"`. The 2026-05-07 veto-removal discussion entry below is **superseded** by this rewrite — FD-14 ADOPTED. Files refactored: `core/_diagnostic_registry.py`, `core/models_compiler.py`, `compiler/merge_service.py` (rewritten as ordered fold), `validator/functions.py` (VE-27 hook reserved), `audit/writers.py` (provenance text format updated). Deleted: `tests/unit/compiler/test_aggregate.py`, `tests/unit/compiler/test_per_source.py`. Added: 4 overlay fixtures under `tests/fixtures/overlay_*` (replace, remove_only, two_features_same_file, no_op_exclude) wired into `tests/integration/test_runner_localfs_e2e.py`. TESTING_STRATEGY.md scenarios 23–28 rewritten for overlay model. Final gate: **1071 passed, 7 skipped, 0 failed**. **Pending follow-ups:** (1) implement VE-27 emission in validator (currently best-effort fixture passes through silently), (2) GitNexus reindex (`npx` not on Windows session PATH — must be done from a tcsh/bash session or after `nodejs` PATH fix; index stale at commit 7672b63). README changelog + pyproject version bumped to `2.0.0a0`.
