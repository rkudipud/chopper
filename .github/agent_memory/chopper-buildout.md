...
- 2026-05-07 Feature-removes-base discussion: current F1/F2 semantics cannot remove base-contributed files/procs from a feature; cross-source FE/PE are vetoed with `VW-19` / `VW-18`. Added `FD-14 Feature Replacement Semantics` to `technical_docs/IMPLEMENTATION.md` as the proposal route. Recommended immediate modeling workaround: keep base strictly common and move replaceable defaults into mutually exclusive features. Recommended future design if approved: explicit opt-in replacement/removal surface, not destructive reinterpretation of existing `files.exclude` / `procedures.exclude`.
