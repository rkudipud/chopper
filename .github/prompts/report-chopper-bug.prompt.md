...
1. Ask me for the `.chopper/` bundle from the failing run (or the full command + terminal output if the run never produced one). If I only have Unix filesystem paths to a `.chopper/` directory, log, or markdown report, package them first with `python scripts/package_bug_report.py <paths...>`.
...
4. Produce the final issue fields and write a payload JSON for `python scripts/file_bug_report.py --payload <payload> --create`. Include:
   - a short issue title
...
5. Run `python scripts/file_bug_report.py --payload <payload> --create` so the GitHub issue is created automatically when `gh` is available and authenticated.
