...
- [`technical_docs/IMPLEMENTATION.md` (pitfalls)](../technical_docs/IMPLEMENTATION.md) -- `P-01` through `P-36` pitfalls and `TC-01` through `TC-10` technical risks.
- [`technical_docs/IMPLEMENTATION.md` (parser section)](../technical_docs/IMPLEMENTATION.md) -- tokenization rules, proc detection algorithm, canonical-name vectors.
- [`tests/TESTING_STRATEGY.md`](TESTING_STRATEGY.md) -- 30-row named integration-scenario catalog (Sec.5).
...
|---|---|---|
| [`fixtures/edge_cases/`](fixtures/edge_cases/) | Adversarial single-file Tcl inputs for parser unit tests | **Populated** -- 22 fixtures (`parser_*.tcl`). See [Sec.2](#2-edge-cases-parser-fixtures). |
...
| [`fixtures/tracing_domain/`](fixtures/tracing_domain/) | BFS trace fixtures (direct, cycle, ambiguous, dynamic) | **Populated** -- 6 fixtures (see [Sec.3](#3-tracing-domain-fixtures)). |
...
| Fixture | Covers | Diagnostic | Spec Sec. |
|---|---|---|---|
| `parser_basic_single_proc.tcl` | Baseline -- `proc foo {} {}` | -- | Sec.4 |
| `parser_basic_multiple_procs.tcl` | Multi-proc file, index stability | -- | Sec.4 |
| `parser_empty_file.tcl` | Zero procs | -- | Sec.2.1.1 row 1 |
| `parser_empty_proc_body_forms.tcl` | `proc foo {} {}` vs `proc foo {args} {}` | -- | Sec.4.1 |
| `parser_brace_in_string_literal.tcl` | P-01: quote context inside braced body | -- | Sec.3.3.1, Sec.3.3.2 |
| `parser_backslash_line_continuation.tcl` | P-02: `\\\n` continuation, line-count integrity | `PW-05` | Sec.3.2 |
| `parser_comment_with_braces_ignored.tcl` | P-34: `#` comments with `{` / `}` inside | -- | Sec.3.4 |
| `parser_nested_namespace_accumulates.tcl` | P-03: nested `namespace eval` composition | -- | Sec.4.5 |
| `parser_namespace_reset_after_block.tcl` | P-03 corollary: namespace stack pops on `}` | -- | Sec.4.5.1 |
| `parser_namespace_absolute_override.tcl` | `::abs::name` overrides active namespace | -- | Sec.4.3 (rows 4-6) |
| `parser_proc_inside_if_block.tcl` | P-04: proc inside `if` body is NOT indexed | -- | Sec.4.4 |
| `parser_computed_proc_name_skipped.tcl` | P-04: `proc $name {...}` skip + warn | `PW-01` | Sec.4.3 (row 6) |
| `parser_duplicate_proc_definition_error.tcl` | P-05: last-wins + `PE-01` | `PE-01` | Sec.2.1.1 row 3 |
| `parser_encoding_latin1_fallback.tcl` | P-09: UTF-8 fail -> Latin-1 retry | `PW-02` | Sec.2 |
| `parser_call_extraction.tcl` | Call tokens extracted (bracketed, bare, namespace-qualified) | -- | Sec.5 |
| `parser_eda_complex_del_seq_rpt.tcl` | Real EDA-style proc with DPA + structured comments | `PI-01`, `PI-04` | Sec.4.6, Sec.4.7 |
| `parser_eda_complex_get_hier_summary.tcl` | Real EDA-style proc, complex body | -- | Sec.4.6 |
| `parser_literal_quote_in_braced_word.tcl` | P-01a: literal `"` inside brace word (`set q {"}`) | -- | Sec.1.3.3 |
| `parser_braced_word_multi_quote_pairs.tcl` | P-01a extension: brace word with multiple quote pairs (`string map {" " ""}`) | -- | Sec.1.3.3 |
| `parser_braced_word_quote_space_quote.tcl` | P-01a extension: brace data word `{" "}` (quote-space-quote) stays literal; lone `}` in quoted word + TW-02 `;`-in-quote regression guards | -- | Sec.1.3.3 |
| `parser_escaped_bracket_in_string.tcl` | P-46: escaped `\[` inside double-quoted string not treated as bracket substitution | -- | Sec.3.2 |
| `parser_switch_brace_pattern.tcl` | P-47: brace-delimited switch pattern with `[...]` inside suppressed as opaque | -- | Sec.5.3 |
| `indent_backslash_continuation.tcl` | P5c formatter -- backslash continuation indent, multi-continuation runs, escaped `\\\\` not treated as continuation | -- | trimmer Sec.5.5 (P5c) |
| `indent_comment_braces.tcl` | P5c formatter -- `#` comment lines with unmatched `{` / `}` and trailing `\` must not shift indent or propagate continuation (real-world `default_fm_procs.tcl` patterns) | -- | trimmer Sec.5.5 (P5c) |
| `indent_quoted_braces.tcl` | P5c formatter -- unmatched `{` inside `"..."` strings and nested braces in `set_vclp_setup_commands` arguments must not affect brace counting (real-world `fev_formality` patterns) | -- | trimmer Sec.5.5 (P5c) |
...
| P-46 | Escaped `\[` false bracket substitution | `parser_escaped_bracket_in_string.tcl` |
| P-47 | Brace-delimited switch patterns with `[...]` | `parser_switch_brace_pattern.tcl` |
| P-48 | Missing Tcl builtins in `TCL_BUILTINS` constant | *(inline -- `TestMissingBuiltins` in `test_call_extractor.py`)* |

### 2.2 Gaps to fill before Stage 1 freeze

The following pitfalls from [`technical_docs/IMPLEMENTATION.md` (pitfalls)](../technical_docs/IMPLEMENTATION.md) do not yet have a dedicated `edge_cases/` fixture. They are **Stage 1 prerequisites** -- author them as part of Stage 1 kickoff:
...
**Acceptance criterion for Stage 1:** every row above has a fixture and a matching unit test asserting both the returned `ProcEntry` set and the emitted diagnostic set. Rows not listed here are covered by existing fixtures in Sec.2 or are pure control-flow variants that the parser's state-machine tests already exercise.
...
This document is a living tracker. When a gap in Sec.2.2 or Sec.3.1 is filled:
...
When every row in Sec.2.2 is filled, Stage 1 is cleared for freeze. When every row in Sec.3.1 is filled, Stage 2 is cleared for freeze.
