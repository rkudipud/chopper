...
| Claim | Source | Test |
| --- | --- | --- |
| Re-trim from `<domain>_backup/` cwd does not nest backups | Sec.5.1 / Sec.2.8 | [tests/integration/test_cli_e2e.py](integration/test_cli_e2e.py#L125), [tests/unit/cli/test_commands.py](unit/cli/test_commands.py#L54) |
| `VE-21` exits 2 when neither domain nor backup exists | Sec.2.8 Case 4 | [tests/unit/orchestrator/test_runner.py](unit/orchestrator/test_runner.py#L63), [tests/unit/orchestrator/test_domain_state.py](unit/orchestrator/test_domain_state.py#L54) |
| `VE-17` is case-insensitive (`casefold()`) | Sec.5.1 | [tests/unit/validator/test_validator.py](unit/validator/test_validator.py#L250) |
| Cycle in tracing emits `TW-04` and terminates via visited-set | Sec.5.4 | [tests/unit/compiler/test_tracer.py](unit/compiler/test_tracer.py#L421) |
| BFS frontier sort determinism under input shuffling | Sec.5.4 | [tests/property/test_determinism.py](property/test_determinism.py#L154) |
| `--strict` + warning escalates to exit 1 | Sec.5.2 | [tests/unit/orchestrator/test_runner.py](unit/orchestrator/test_runner.py#L169) |
| Cleanup without `--confirm` exits 2 | Sec.2.8 / cleanup | [tests/integration/test_cli_e2e.py](integration/test_cli_e2e.py) `TestCleanupSubcommand` |
| Layer-shadow transition emits `VW-21` | Sec.4 R1 (ordered overlay) | [tests/unit/compiler/test_merge_service.py](unit/compiler/test_merge_service.py) `test_vw21_emitted_when_feature_pi_overrides_base_pe` |
| Same-source FE/PE conflict emits `VW-11` | Sec.4 R1 (same-layer rules) | [tests/unit/compiler/test_merge_service.py](unit/compiler/test_merge_service.py) `test_vw13_with_pi_redundant_emits_vw09_too` and adjacent VW-11/12/13 cases |
| Tool-command pool downgrades `TW-02` -> `TI-01` | Sec.3.10 | [tests/unit/compiler/test_tracer.py](unit/compiler/test_tracer.py#L286) |

### Gap

| Tier | Claim | Source | Resolution |
| --- | --- | --- | --- |
| P1 | `--strict` does not rewrite the `severity` field of `VW-*` diagnostics -- only the exit code changes. | Sec.5.2 / DIAGNOSTIC_CODES Notes | **Closed in this pass** -- `tests/unit/orchestrator/test_runner.py::TestStrictMode::test_strict_does_not_rewrite_warning_severity` |
| P1 | `--dry-run` never creates `<domain>_backup/`. | Sec.3.7 / Sec.2.8 | **Closed in this pass** -- `tests/integration/test_cli_e2e.py::TestTrimSubcommand::test_trim_dry_run_does_not_create_backup_directory` |
...
| P2 | `--tool-commands` is repeatable and accumulates pool entries. | Sec.3.10 | Covered in tracer pool tests + `tests/unit/cli/test_commands.py` (build_run_config tests) -- left under Covered. |
| P3 | Render-formatting niceties (color flags, plain-mode text). | CLI Sec.global flags | Out of scope for this pass. |
...
| Claim | Source | Test |
| --- | --- | --- |
| UTF-8 decode -> Latin-1 fallback emits `PW-02` | Sec.1.2 | [tests/fixtures/edge_cases/parser_encoding_latin1_fallback.tcl](fixtures/edge_cases/parser_encoding_latin1_fallback.tcl) |
| Backslash line continuation does not eat lines | Sec.1.3.2 | edge-case fixture `parser_backslash_line_continuation.tcl` |
| Duplicate proc -> `PE-01`, last definition wins | Sec.1.4.9 D-1d-02 | parser tests + edge-case fixture |
| DPA `\`-continuation does not absorb args into name | Sec.1.4.6 | [tests/unit/parser/test_bug_report_regressions.py](unit/parser/test_bug_report_regressions.py#L145) |
| Canonical proc-name format vectors | Sec.1.4.3.1 | [tests/unit/core/test_models.py](unit/core/test_models.py) |
| Brace-depth-3 namespace (worked example) | Sec.1.4.5.1 | parser tests |
| Lex-sort BFS frontier (NFR-03) | Sec.1.8.5.2 | property test (linked above) |

### Gap

| Tier | Claim | Source | Resolution |
| --- | --- | --- | --- |
| P2 | DPA association tolerates <=3 blank lines between proc close and DPA block; 4 blank lines breaks the window. | Sec.1.4.6 rule 2 | **Closed** -- `tests/unit/parser/test_proc_extractor.py::TestDPA::test_dpa_blank_line_tolerance_boundary` (parametrized over 0/1/2/3/4 blank lines, asserts attach for 0-3 and no-attach for 4). |
| P3 | Comment-banner backward scan stops at first blank line. | Sec.1.4.7 | **Closed** -- `tests/unit/parser/test_proc_extractor.py::TestCommentBanner::test_blank_line_breaks_banner` (explicit blank-line break) and `test_banner_stops_at_code` (non-comment break). |
...
Re-grepped `tests/` for forbidden tokens listed in `.github/instructions/project.instructions.md` Sec.4
...
- **Crash-injection scenarios 5-9** -- recorded as deferred in [TESTING_STRATEGY.md](TESTING_STRATEGY.md) per architecture Sec.2.8. Out of scope.
