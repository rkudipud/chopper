...
def test_p4_skipped_on_retrim_case_2() -> None:
    from chopper.adapters import InMemoryFS
    ...
    assert report.p4_checkout.skip_reason is not None
    assert "previous trim" in report.p4_checkout.skip_reason
    mock_available.assert_not_called()
    mock_checkout.assert_not_called()
    # Normal trim still proceeds and succeeds.
...
def test_p4_checkout_failure_aborts_reverts_and_leaves_domain_untouched() -> None:
    ...
    assert report.p4_checkout.domain_restored is False  # nothing was ever renamed
    assert "VE-37" in sink.codes()

    # Nothing was renamed or rewritten -- domain untouched, no backup created.
