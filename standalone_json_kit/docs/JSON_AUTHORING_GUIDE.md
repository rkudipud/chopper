...
When stages are defined in a base or feature JSON, Chopper can generate a stack file where each JSON stage maps to one stack-file node. The mapping is direct and deterministic:
...
> `load_from` feeds the generated `run_<stage>.tcl` script (data sourcing, `ivar(src_task)` semantics). It is **not** the stack `D` line.  
...
| `load_from` | Yes | — | Data predecessor for run script; can be empty string |
| `steps` | Yes | — | Ordered step strings written into run_\<stage\>.tcl |
...
| `replace_step` | `stage`, `replace`, `with` | Replace one step with another |
...
| `replace_stage` | `replace`, `with` | Replace a stage with a new definition |
...
```json
// load_from = data predecessor for the run script (which ivar task to read from)
// dependencies = stack scheduler D line (which task must complete first)
// They can differ! Example:
{
  "name": "eco_targ_synth",
  "load_from": "eco_pre_synth",       // run script reads from eco_pre_synth
