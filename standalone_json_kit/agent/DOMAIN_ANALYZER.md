...
1. **Discover** what exists — files, proc libraries, run scripts, scheduler stack files, config files, helper utilities
2. **Classify** each discovered artifact — core (always needed) vs. optional/scenario-specific
3. **Map** scheduler stack entries to JSON stage definitions
...
You do **not** run any tool commands or execute Chopper. You read files, reason about their relationships, and produce JSON.
...
### Q2 — What scheduler stack files exist?

Stack files define the job execution graph. They contain stage names, commands, dependencies, legal exit codes, inputs, and outputs. Look for:
...
## Phase 2 — Extract Stage Definitions from Stack Files

**Goal:** Translate scheduler stack entries into JSON `stageDefinition` objects.
...
Each stage typically has a corresponding run script (often named after the stage, e.g., `run_<stage_name>.tcl` or `<stage_name>.tcl`).  
...
2. **Ask to see stack files before run scripts.** Stages are the skeleton; scripts fill in the steps.
