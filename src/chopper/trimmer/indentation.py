...
This module ports the legacy Perl brace-driven formatter into
Python and applies it only to emitted ``.tcl`` outputs after P5a trim and
P5b generation have written files into the rebuilt domain.
"""
...
_NORMALIZED_TREATMENTS = frozenset({FileTreatment.FULL_COPY, FileTreatment.PROC_TRIM, FileTreatment.GENERATED})
...
def _with_updated_bytes(report: TrimReport, normalized: dict[Path, str]) -> TrimReport:
    ...
    for outcome in report.outcomes:
        ...
        if text is None or outcome.treatment not in (FileTreatment.FULL_COPY, FileTreatment.PROC_TRIM):
