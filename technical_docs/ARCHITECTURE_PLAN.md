...
- **No `plugins/`, no `advisor/` directory exists or is planned.** These are not v1-deferred features — they are permanently excluded (see §16 Q1). Any PR that creates such a directory is rejected without review. The `src/chopper/mcp/` directory is the **narrow, read-only** MCP surface introduced in 0.4.0 (bible §3.9) and is permitted by an explicit amendment to the scope-lock; it does not open the door to `adapters/mcp_*.py` or any destructive MCP surface.
