...
**Single-domain output:**
```
The output logs of this run can be found at: /ward/global/snps/fev_formality/.chopper
```
...
**Gating — only a genuine first trim.** Checkout only runs when `DomainState.case == 1` (domain exists, no `<domain>_backup/` yet — see §5.2 domain-state cases). On a re-trim (cases 2/3) `domain_root` no longer holds the original p4-synced files (it holds the previous trim's rebuilt output, or does not yet exist), so running `p4 edit` there would check out the wrong content or fail outright. Checkout is skipped with a reason in this case, not attempted.

**Detection.** Two gates, deliberately simple (no per-file `p4 where` probe): (1) the `p4` binary must be resolvable on `PATH`; (2) `p4 info` must succeed (exit 0) when run with `cwd=domain_root`. If either gate fails, or gating above skips the case-2/3 scenario, Chopper prints a red, screen-visible notice to stderr (degrading to plain text under `--plain` or a non-terminal stream) explaining why, and the trim proceeds **normally** without any `p4` interaction — this is advisory only, never an error, never a non-zero exit.

**Where it hooks in, and why order matters.** `TrimmerService.run()` renames `domain/` → `domain_backup/` and rebuilds a **fresh** `domain/` from scratch (per-file writes land in brand-new files at the same paths, never in-place edits of the synced files). Perforce tracks "opened" state by client path, not inode, so checkout **must** happen before that rename — while the real synced files still sit at their real client-mapped paths. This is why checkout is the very first thing `TrimmerService.run()` does after the dry-run short-circuit, strictly before `_prepare_workspace()`.

**Which files.** The exact `PROC_TRIM` + regenerate-in-place `GENERATED` file set (same semantic family as the `p4_commands.txt` edit section), resolved against `domain_root` directly (not `backup_root`, since this runs before the rename). Newly-created `GENERATED` files (no pre-trim counterpart) are excluded — they don't exist on disk yet, so there is nothing to check out; Chopper never runs `p4 add`. `p4 edit -t text+x <path>` is the only command issued.
...
| FR-53 | `chopper trim --p4` (opt-in, live trim only, `trim`-subcommand-only CLI flag, no JSON field) runs `p4 edit -t text+x` on every `PROC_TRIM` / regenerate-in-place `GENERATED` file before P5 rebuilds the domain, so a human's later `p4 submit` doesn't fight Perforce's checkout-before-edit protocol. Only attempted on a genuine first trim (`DomainState.case == 1`); skipped with an on-screen notice (not an error) when `p4` is unavailable, the domain is not a working p4 client workspace, or this is a re-trim. On checkout failure the whole trim aborts (`VE-37`) after `p4 revert` on whatever succeeded; on a later P5 failure after checkout succeeded, rollback additionally restores `domain/` from `domain_backup/` immediately. Chopper never runs `p4 add`, `p4 delete`, or `p4 submit` here. See §5.5.18. |
