...
7. **Machine output is deferred, not promised.** Chopper does not freeze a `--json` CLI mode or a GUI wire protocol here. If structured stdout/stderr is needed later, it enters through [`FD-10`](IMPLEMENTATION.md Appendix B#fd-10-machine-readable-cli-output), not via an undocumented side channel.
...
2. **Structured CLI transport** added later through [`FD-10`](IMPLEMENTATION.md Appendix B#fd-10-machine-readable-cli-output) once the pipeline and artifact shapes have proven stable.
...
- **Out-of-v1 roadmap.** Planned but non-v1 capabilities are tracked in [technical_docs/IMPLEMENTATION.md Appendix B](IMPLEMENTATION.md). Nothing in this document commits v1 to those items.
...
| [technical_docs/IMPLEMENTATION.md Appendix B](IMPLEMENTATION.md) | Roadmap items explicitly out of v1 scope |
...
| 2026-05-22 | **3.4.1 — P5d companion-file sync (FD-15) + documentation refresh.** (1) **`CompanionSyncService`** (`src/chopper/trimmer/companion_sync.py`) runs after P5c in the P5 pipeline. For every `PROC_TRIM` `default_rules.<sfx>.tcl`, it locates sibling `default_config.<sfx>.csv` and `default_milestone.<sfx>.tcl` companion files. CSV companions are row-filtered (column 0 checked against surviving proc short-names); milestone companions are line-filtered (`change_config <ProcName>` lines checked). Surviving procs are derived from `manifest.proc_decisions` PI set for that rules file. `VW-24 companion-file-missing` emitted when a companion is absent from the rebuilt domain; `VI-04 companion-sync-applied` on success. Returns an updated `TrimReport` with corrected `bytes_out` for each modified companion. Gated on `not ctx.config.dry_run`. (2) **Diagnostic registry:** VW-24 (trimmer, P5, exit 0), VI-04 (trimmer, P5, exit 0). Active count 79→81, reserved VW 7→6, VI 4→3. Grand total 79→81 active / 42 reserved. (3) **Documentation refresh.** All technical and user docs updated to reflect the current implementation: ENGINEERING.md §3 module layout rewritten to match actual source tree (all 14 packages, all files); tool-command pool descriptions updated from single PrimeTime seed to 6 bundled pools (pt, pwr, pe, ps, fm, pc); `user_docs/02_CLI_GUIDE.md` live-trim steps expanded with P5d and JSON-input-preservation sub-steps; `user_docs/03_HOW_CHOPPER_WORKS.md` module map and P5 pipeline table refreshed; `user_docs/01_OVERVIEW.md` BKM and flag descriptions updated; `project.instructions.md` module table updated. `pyproject.toml` 3.4.0 → 3.4.1; README changelog mirrors. |
| 2026-05-21 | **3.4.0 — F3 topological aggregate stack + `standalone_stack` suppresses `<stage>.tcl`.** Two semantic refinements to the 3.3.0 stack work, hard cutover (no JSON knob, no backward-compat shim). (1) **Topological record order in `<basename(domain_root)>.stack`.** Aggregate-stack records were previously emitted in `manifest.stages` declared order (R1 ordered-overlay output). They are now emitted in topological order over the stage dependency graph, where edges are `dependencies ∪ {load_from}` per stage. Kahn's algorithm with **authored position** as the tiebreaker (lowest authored index wins among equal in-degree nodes) — fully deterministic, preserves intuitive authoring intent for unrelated subgraphs, no lex shuffle. The new order is materialized on `CompiledManifest.stack_order: tuple[str, ...]`; `manifest.stages` itself remains in authored order so `<stage>.tcl` emission, trim report, and audit views continue to use authored sequencing. `run_mode` (serial/parallel) is unaffected by ordering — it remains record-local metadata surfaced via the `R parallel` line only. (2) **`standalone_stack: true` now suppresses `<stage>.tcl`.** Previously a standalone-stack stage emitted both `<stage>.tcl` and `<stage>.stack`. The two were intended as alternative drivers (`.tcl` for generated Tcl, `.stack` for verbatim scheduler records), and emitting both forced authors to keep redundant content in sync. With 3.4.0, a stage with `standalone_stack: true` emits **only** `<stage>.stack` (plus a record in the aggregate `<domain>.stack` when `generate_stack` is on); no `<stage>.tcl` is produced for that stage. (3) **New diagnostics.** `VE-30 stage-dependency-cycle` (exit 1, phase 3) and `VE-31 stage-dependency-unresolved` (exit 1, phase 3) are emitted whenever `stages` is non-empty, regardless of `generate_stack` — a malformed dependency graph is an authoring bug independent of stack emission. Both raise `ChopperError` to abort compilation, consistent with `VE-28`/`VE-29` collision handling. VE band extended VE-30 → VE-35 (active 29→31, reserved 1→4). (4) **Cascades.** Architecture doc §3.1 (options row), §3.6 (intro, behavior table, stageDefinition table, full rewrite of aggregate-stack subsection with graph construction + Kahn's algorithm + tiebreaker rule + new diagnostics + standalone subsection + truth table), §6.6 (mapping paragraph) — all rewritten in place. `technical_docs/DIAGNOSTIC_CODES.md`: VE summary row, VE band header, VE-30 / VE-31 rows added (in lowest reserved slots); grand totals adjusted from 77 active / 42 reserved / 121 total to **79 / 44 / 125** (the prior +1 drift in reserved/total was corrected in the same pass). `technical_docs/JSON_AUTHORING_GUIDE.md` stack section updated to document topological ordering and the standalone-suppresses-tcl rule. (5) **Code.** `CompiledManifest` gains `stack_order: tuple[str, ...] = ()` (frozen, post-init asserts permutation of `{s.name for s in stages}` when non-empty). New `_compute_stack_order(ctx, stages)` in `src/chopper/compiler/stack_graph.py` builds the graph, runs Kahn's algorithm with `(in_degree, authored_index)` priority, and emits `VE-30`/`VE-31` before raising `ChopperError`. `CompilerService.run` calls it once after `resolve_stages` and threads the result into `CompiledManifest.stack_order`. `emit_flow_stack` signature extended to `(stages, domain_name, stack_order=())`; when `stack_order` is non-empty, records are emitted by looking up stages by name in that order. `GeneratorService.run` skips `emit_stage_tcl` when `stage.standalone_stack is True` and passes `manifest.stack_order` to the aggregate emitter. `merge_service._register_generated_stage_files` drops the `<stage>.tcl` registration for standalone stages (so file-decision collision checks don't false-positive on a path that will never be emitted). `_diagnostic_registry.py` gains `VE-30`/`VE-31` entries. (6) **Tests.** New unit tests for cycle detection (`VE-30`), unresolved-ref detection (`VE-31`), `dependencies ∪ load_from` graph construction, Kahn's algorithm correctness with authored-position tiebreaker, topological permutation invariant on `CompiledManifest.stack_order`, `standalone_stack: true` suppresses `<stage>.tcl` (negative assertion on artifact list). Existing aggregate-stack goldens updated to reflect topological order where it differs from authored order. Diagnostic registry count test updated 77→79. (7) **Non-stack regression protection.** F1, F2, parser, trimmer (`PROC_TRIM` / `FULL_COPY` / `REMOVE`), validator pre/post, MCP, audit-bundle structure, exit-code policy, and CLI surface unchanged. Tcl golden files unchanged. `pyproject.toml` version bumped 3.3.0 → 3.4.0; README changelog mirrors. |
| 2026-05-21 | **3.3.0 — F3 aggregate `<domain>.stack` + per-stage `standalone_stack`.** Two coordinated F3 changes, no backward compatibility. (1) **Aggregate stack correction.** Reference EDA domains (e.g. `fev_formality`) ship one scheduler stack file per flow (`fev_fm_eco.stack`, `fev_fm_lite.stack`, etc.) containing all stage records concatenated with a single Intel header at top and records separated by a blank line — not the per-stage `<stage>.stack` files Chopper had been emitting since 2026-04-24. `options.generate_stack: true` now emits exactly **one** aggregate stack file at the domain root named `<basename(domain_root)>.stack` (filename sourced from the runtime domain-root basename — no JSON field), containing one record per resolved stage in `manifest.stages` declared order. Per-record line order corrected to `N → J → L → I → O → D → (R parallel)` matching production. `R` line emitted **only** when `run_mode == "parallel"`; serial is implicit (matches production convention — no production `.stack` file emits an `R` line). The per-record `# Chopper-generated stack: <name>` provenance banner is kept. The old per-stage `<stage>.stack` emission and `stack_output_path()` helper were deleted outright; no compatibility shim. (2) **`standalone_stack: true` per-stage flag.** Optional boolean on each `stageDefinition` (default `false`). When `true`, F3 emits an additional `<stage>.stack` whose body is the Intel header followed by `"\n".join(stage.steps)` — the authored `steps` verbatim, no record derivation. Orthogonal to `options.generate_stack` and to `<stage>.tcl` emission: the stage still emits its `.tcl` and (when `generate_stack` is on) is still included in the aggregate as a derived record. Use case: wrapper stages whose `steps` are themselves a verbatim scheduler-format record (ECO hooks, scheduler-only sub-flows). (3) **New diagnostics.** `VE-28 aggregate-stack-collision` (aggregate `<domain>.stack` collides with `files.*` entry, exit 1, phase 3); `VE-29 standalone-stack-collision` (per-stage `<stage>.stack` collides with `files.*` or with the aggregate path, exit 1, phase 3); `VW-23 stack-stage-empty-command` (a stage in the aggregate has empty `command` — scheduler will likely reject the record, exit 0, phase 3). Registry summary counts updated: VE 27→29 active / 3→1 reserved; VW 20→21 active / 8→7 reserved. (4) **Cascades.** Architecture doc §3.1 (options table), §3.6 (intro paragraph + behavior table + stageDefinition table with `standalone_stack` row + full rewrite of stack subsections + truth table), §6.6 (stack mapping paragraph), §6.6.1 (Intel header note covering all three artifact paths), §10 OQ-01 resolution, glossary "Stack file" entry — all rewritten in place (no addendums). JSON authoring guide §2.1 stack-format snippet rewritten with the aggregate shape, an `R`-suppression note, and a `standalone_stack` example. New diagnostic-codes rows added in lowest-available reserved slots. Schema `base-v1`: `options.generate_stack` description rewritten; new `standalone_stack` boolean property added to `stageDefinition` with default `false`. (5) **Code.** `src/chopper/generators/stack_emitter.py` rewritten — `emit_stage_stack` / `stack_output_path` removed; `emit_flow_stack(stages, domain_name)` (returns `GeneratedArtifact | None`; `None` only when stages is empty) and `emit_standalone_stack(stage)` added. `GeneratorService.run` iterates stages emitting `<stage>.tcl` always, additionally `<stage>.stack` for standalone stages, then emits the aggregate after the loop. `merge_service._register_generated_stage_files` registers `<stage>.tcl` always, additionally `<stage>.stack` for standalone stages, plus the aggregate path once when `generate_stack` is on. `StageSpec` and `StageDefinition` gain `standalone_stack: bool = False`. `BaseOptions.generate_stack` semantics unchanged at the dataclass level; only the emitter behavior changed. (6) **Tests.** Old per-stage stack unit + integration assertions deleted; new tests added covering aggregate emission, record-line order, R-suppression, D-derivation, standalone verbatim emission, all four truth-table combinations, collision diagnostics, and `VW-23` empty-command warning. `tests/fixtures/stages_domain/` refreshed to assert the aggregate shape; new mixed-flow fixture exercises standalone. Two new example folders added — `examples/12_base_with_aggregate_stack/` and `examples/13_base_with_standalone_stack/` — demonstrating the new behaviors end-to-end. (7) **Non-stack regression protection.** F1, F2, parser, trimmer (`PROC_TRIM` / `FULL_COPY` / `REMOVE`), MCP, audit-bundle structure, exit-code policy, and CLI surface unchanged. Tcl golden files unchanged. `pyproject.toml` version bumped 3.2.1 → 3.3.0; README changelog mirrors. |
| 2026-05-20 | **3.2.1 — `VW-22 proc-trim-no-drop` (issue #26).** New P5 trim warning emitted when a `PROC_TRIM` file's backup contains zero procs to drop — the drop set is empty because every proc found in the backup already belongs to the keep set. The canonical cause is a stale backup that holds a prior run's post-trim output rather than the original pre-trim source (regression or `cleanup` removed the real backup; a subsequent Case 1 run promoted the trimmed domain to backup; the second trim has nothing more to do). New `VW-22 proc-trim-no-drop` row in `technical_docs/DIAGNOSTIC_CODES.md`; new `_emit_vw22()` helper in `src/chopper/trimmer/service.py` emits from `_dispatch` when `outcome.procs_removed` is empty on a live `PROC_TRIM` outcome. Architecture doc §5.2.1 P5 PROC_TRIM bullet extended with the zero-drop rule; §11.6 Corner Case FAQ gains a new Q&A entry. `pyproject.toml` version bumped 3.2.0 → 3.2.1. |
| 2026-05-19 | **3.2.0 — Coverage gate raised to 99% and validated at 100%.** Updated the active coverage policy from 78% to 99% in canonical testing/config docs and in pytest coverage enforcement (`pyproject.toml` `--cov-fail-under=99`). Full regression gate run (`make ci`) completed green with **1410 passed, 0 failed** and **100.00% total coverage** (5512 statements, 0 missed; 1912 branches, 0 partial). Cascaded updates applied to `tests/TESTING_STRATEGY.md`, `README.md` changelog, and project instruction docs so user-facing and contributor-facing guidance agree on the new threshold. No diagnostic-registry, schema, CLI surface, exit-code, or pipeline-phase behavior changes. Version bumped 3.1.0 → 3.2.0. |
...
| 2026-05-12 | **2.9.0 — Production-hardening refactor (no spec / registry / schema changes).** (1) Single shared full-domain BFS walker: new `src/chopper/core/fs_walk.py` (`walk_files()` + `TEXT_LIKE_EXTENSIONS`) consumed by both the audit summary (`audit/writers.py`) and `chopper loc` (`cli/loc_report.py`), eliminating drift between the two enumerations. File-count buckets walk every regular file; SLOC math is constrained to `TEXT_LIKE_EXTENSIONS`. (2) Batched `cloc` subprocess: `audit/cloc_backend.py::count_sloc_via_cloc_batch()` runs one `perl cloc.pl --by-file --json --quiet` per tree; `audit/sloc.py` exposes `count_sloc_many()`. Hot domains go from O(N) subprocess starts to O(1). Whitespace-only payloads short-circuit to 0 without invoking cloc. Single-file `count_sloc()` API unchanged. (3) Symmetric trim-stats totals: `cli/render.py::_totals_row()` is unconditionally labelled `TOTAL` (no live-vs-dry-run divergence); `_collect_rows()` falls back to `domain_root` for `sloc_in` under `--dry-run` so the two modes produce byte-identical tables. (4) `files_skipped_decode: int` added to `chopper_run.json` summary and `trim_stats.json` — counts SLOC-eligible files that fail to decode (binary / bad encoding / OS error). Additive field only; previously these files were silently treated as 0-SLOC. (5) Cross-phase exec-bit helper hoisted: `ensure_executable()` + `EXEC_BITS` moved from `trimmer/file_writer.py` to new `src/chopper/core/file_perms.py`, plus new `mirror_perms_plus_exec(src, dst)` helper. Every rebuilt file in `<domain>/` now carries the source file's full mode bits (`shutil.copymode` semantics: read/write/setuid/sticky etc.) plus `a+x` for user/group/other. Uniformly applied to `FULL_COPY`, `PROC_TRIM`, and regenerate-in-place `GENERATED`; pure-new `GENERATED` artifacts (no source counterpart) get `a+x` only. Removes the generator→trimmer sibling import; `lint-imports` `Services are independent` contract now green. (6) Python 3.13 baseline: `pyproject.toml` `requires-python = ">=3.13"`, ruff `target-version = "py313"`, mypy `python_version = "3.13"`. (7) `trimmer/indentation.py` docstrings: dropped the misleading "ports the legacy Perl formatter" framing (no `indent.pl` ships with Chopper; brace-driven Python-native algorithm). Gate status: ruff lint + format clean, mypy clean (78 files), `lint-imports` 4/4 contracts kept, pytest 1097 passed / 10 skipped, coverage 94.34%. No diagnostic-registry, schema, CLI, or audit-bundle field-shape changes (additive `files_skipped_decode` only). |
| 2026-05-11 | **2.6.4 — `chopper trim` UX cluster (no spec/registry/schema changes).** (1) New post-run stats table in `cli/render.py::render_trim_stats()` — console-width-aware, columns `File / Op / SLOC (in → out) / Procs (kept/dropped)` plus a `TOTAL` row, rendered to stderr after `Summary:` on live `chopper trim`. Generated artifacts are folded in; when a generated path also existed in the source domain (`fev_fm_rtl2gate.tcl`-style regenerate-in-place) the `in` SLOC baseline comes from `<domain>_backup/` so the delta is real, not 0→N. (2) `chopper loc` aligned: `cli/loc_report.py` GENERATED bucket now captures pre-existing source as `lines_before`/`sloc_before`, and `files.before` counts the regenerate-in-place path once. (3) P5a/P5b now `chmod a+x` every file written to `<domain>/` (shared `ensure_executable()` in `core/file_perms.py`); errors swallowed so NFS exports that reject `chmod` do not break the trim. (4) `TI-01 known-tool-command` suppressed from stderr by `render_diagnostics()` (`_SUPPRESSED_STDERR_CODES`) — kept in audit bundle unchanged; severity/registry/exit-code policy untouched; `--strict` unaffected. (5) NFS-stale-cwd pre-flight notice in `cmd_trim` when cwd is inside the soon-to-be-renamed domain and no backup yet exists. (6) `Path.cwd()` hardening — `cli/commands.py::_resolve_domain_root` and `audit/internal_error.py::_resolve_audit_root` now handle `FileNotFoundError`/`OSError` cleanly (stale NFS inode); CLI exits with a readable message; the internal-error writer falls back to `tempfile.gettempdir()/chopper`. `user_docs/02_CLI_GUIDE.md` `## chopper trim` cascaded. No new diagnostic codes, no schema field additions, no audit-bundle shape changes. |
| 2026-05-11 | **2.6.3 — Docs-only patch.** Expanded `chopper loc` documentation to make file-type coverage and caveats explicit for end users. Added a counted-extensions table (`.tcl`, `.py`, `.pl`, `.pm`, `.sh`, `.bash`, `.csh`, `.tcsh`, `.zsh`, `.ksh`, `.json`, `.csv`) with per-language SLOC rules; documented source-root resolution (`backup_root` preferred when present, mirroring parser); listed accounting caveats (PROC_TRIM dry-run reconstruction, all-procs-dropped collapse to REMOVE, P5c not modeled, default-exclude under R2, `.chopper/` excluded, decode fallback, no audit bundle); added worked corner-case examples. Cascaded across §5.7, CLI_REFERENCE.md `## chopper loc`, and user_docs/02_CLI_GUIDE.md. No code, schema, registry, or test changes. |
| 2026-05-11 | **2.6.2 — Patch.** `chopper loc` renderer changed from ASCII table to line-oriented `key: value` output (grep/awk/CI-friendly). VW-10 false-positive fix in `validator/functions.py`: post-trim logical-size normalization now applies only to `PROC_TRIM` outcomes (which record `bytes_out` from LF-normalized text), not to `FULL_COPY` `.tcl` files (which record `bytes_out` from raw `stat()` and so must compare byte-for-byte) — resolves a Windows CRLF false-positive in `_check_trim_output_for_each_outcome`. Runner `finally`-block guard added (`if command != "loc":`) so the audit-skip path no longer clobbers `RunResult` with `None`. User-facing docs (`user_docs/02_CLI_GUIDE.md`) and onboarding presentation (`presentation/chopper_onboarding.html`) cascaded with a dedicated `chopper loc` slide and updated subcommand counts. No spec/registry/schema changes. |
| 2026-05-11 | **2.6.0 — New `chopper loc` read-only LOC report subcommand.** Fifth user-facing subcommand (joining `validate`, `trim`, `cleanup`, `mcp-serve`). Same input flags as `validate`/`trim` (`--base [--features]` or `--project`). Runs P0–P4 + dry-run-P6 plus `GeneratorService` in no-write mode (so generated stage `.tcl` content is countable), then prints a stdout LOC table: files before/after, physical lines before/after, SLOC before/after, percent reduction. Writes **nothing** to the filesystem — no domain modifications and no `.chopper/` audit bundle. The runner now branches on `command == "loc"` to (a) run `GeneratorService` even in dry-run mode and (b) skip the P7 audit `finally` block. Per-treatment line accounting documented in §5.7: `FULL_COPY` reuses source line count; `PROC_TRIM` masks out `procs_removed` line spans (body + DPA + leading comment when present on `ProcEntry`); `REMOVE` is 0; `GENERATED` counts the rendered artifact content. Files present in the source domain but absent from `manifest.file_decisions` are treated as REMOVE for the after totals (default-exclude under R2). Exit-code policy matches `validate` (0/1/2/3); `loc` cannot return `4`. **FR-46** added. **§5.1.1** example block extended with two `chopper loc` invocations. **§5.7** Dry-Run vs Live Trim grew a `chopper loc` paragraph + per-treatment table. **§5.9** subcommand enumeration extended. New `cli/loc_report.py` module owns the math and table render. CLI_REFERENCE.md top-level usage block + new `## chopper loc` section cascaded. `.github/instructions/project.instructions.md` §1 scope-lock row + Core-Modules cli row updated. README changelog entry added. No diagnostic-registry, schema, or audit-bundle surface changes (audit bundle is intentionally not produced by `loc`). Version bumped 2.5.0 → 2.6.0. |
| 2026-05-09 | **2.5.0 — Parser bugfix (P-01a literal-quote-in-braced-data-word) + alpha tag dropped.** The structural tokenizer in `src/chopper/parser/tokenizer.py` was opening a phantom quoted word whenever an unescaped `"` appeared as the first byte inside a literal data word (`set q {"}`, `regexp {".*"} $line`). The phantom quote silently consumed the matching `}` and desynced brace depth, eventually producing a false-positive `PE-02 unbalanced-braces`. Real-world repro: Synopsys Formality `default_fm_procs.tcl` emitted `PE-02` at line 2535 caused by `set q {"}` at line 2480 (verified valid Tcl by `tclsh info complete`). Fix is a one-byte peek-back at quote-open time: if `text[i-1]` is an unescaped structural `{`, the `"` is treated as the literal first content byte of the braced word (Tcl Endekas rule 6) instead of opening a quoted word. Closing `}` handling is unchanged. New regression fixture `tests/fixtures/edge_cases/parser_literal_quote_in_braced_word.tcl` and three new tokenizer unit tests. IMPLEMENTATION.md §1.3.3 "Literal-data-word exception" + §12 P-01a row added in place. Pre-existing baseline cleanups landed in the same release: ruff format drift on `trimmer/indentation.py` reformatted; mypy `Literal` narrowing on `merge_service.py:625` fixed by adding a `Literal["replace", "downgrade-whole-to-trim"]` annotation on the local `action` variable. Alpha tag dropped (was `2.0.0a3`); minor bumped 2.0 → 2.5 to mark the parser as production-validated against the snps Formality real-world fixture. No diagnostic-registry, schema, CLI, or audit-bundle surface changes. |
| 2026-05-09 | **2.0.0a3 — P5c indentation pass made opt-in via `base.options.indent` (default `false`).** Real-world domains were tripping `VW-10` reconciliation and getting misindented because the legacy Perl-port `format_tcl_indentation` formatter has known structural gaps (no quote/comment awareness, no line-continuation folding, single-`}` dedent only, mangles user-authored whitespace inside braced literals). Rather than rip out the pass, the runner now gates it on `loaded.base.options.indent`. When the flag is `false` (the default), `TclIndentationService.run(..., enabled=False)` is a pass-through: it returns `trim_report` and `artifacts` unchanged but still computes the rewritten-path tuple via the new public `tcl_output_paths(manifest)` helper, so P6's `VE-16` brace-balance check keeps covering every PROC_TRIM and GENERATED `.tcl` output. PROC_TRIM and GENERATED outputs reach disk verbatim from P5a/P5b. The formatter algorithm itself was deliberately not rewritten in this release; the inline `options.indent` row in §3.1 and the P5c paragraph in §5.5 record the current limitations and the conditions for opting in. New `options.indent` boolean added to `schemas/base-v1.schema.json` and `BaseOptions`; §3.1 row + §5.5 P5c paragraph updated in place; JSON_AUTHORING_GUIDE §2 options table and ENGINEERING `TclIndentationService` rows cascaded; user_docs (01 overview switches table, 02 CLI guide P5c bullet, 03 module map) updated. No diagnostic-registry, CLI, or audit-bundle surface changes. |
...
| 2026-04-21 | Re-synchronised chopper docs with `json_kit/` 1.0.2, which standardised feature JSON filenames to `<feature_name>.feature.json` and moved example instances under `jsons/` subdirectories. Updated every `examples/*` citation in `ARCHITECTURE.md` (§3.1 base minimal/full examples, §3.2 feature example, §3.3 project example) to reference the `jsons/base.json` and `jsons/features/<name>.feature.json` layout. Replaced stale `feature_<name>.json` tokens in §5.1.1 CLI invocation examples, §5.5 audit-artifact tree (`input_features/01_dft.feature.json`, `02_power.feature.json`), the `chopper_run.json` example (`feature_jsons`), the `compiled_manifest.json` `input_sources` row (keyed by feature `name`, not filename), and the §6.6 extended project example (`scan_common.feature.json`, `dft.feature.json`, `power.feature.json`). Expanded §2.5 per-domain tree to show the `<feature>.feature.json` filename convention directly. No schema fields, diagnostic codes, or merge/trace semantics changed — this was a naming- and path-only alignment pass triggered by the json_kit release, cascading the new convention into every subordinate chopper doc reference. |
| 2026-04-21 | Day-0 handoff-readiness cleanup driven by `technical_docs/DAY0_REVIEW.md` decisions. **Diagnostic registry renumbered for compactness** (F1): `DIAGNOSTIC_CODES.md` rewritten to remove all "retired" slots, since Chopper is still in ideation and no code has ever shipped. `VE-17 brace-error-post-trim` → `VE-16`; `VE-19 project-domain-mismatch` → `VE-17`; `VE-20 duplicate-feature-entry` → `VE-18`; `VE-21 occurrence-suffix-zero` → `VE-19`; `VE-22 ambiguous-step-target` → `VE-20`; `VE-23 no-domain-or-backup` → `VE-21`; `VE-25 feature-depends-on-cycle` → `VE-22`; `VE-26 filesystem-error-during-trim` → `VE-23`; `VE-27 backup-contents-missing` → `VE-24`; `VE-28 domain-write-failed` → `VE-25`; `VE-29 proc-atomic-drop-failed` → `VE-26`. Dropped `VE-16` (old `depends-on-out-of-order`), `VE-18` (old `template-script-path-escapes`), `VE-24` (old concurrent-invocation), `VI-03` (old `domain-hand-edited`), `VI-04`, and `VI-05` entirely — no active codes have historical usage to preserve. **Scope cuts applied** (G1, G2, G3): `options.template_script` field and its diagnostic removed from `schemas/base-v1.schema.json`, `JSON_AUTHORING_GUIDE.md`, and `technical_docs/IMPLEMENTATION.md` (pitfalls) TC-09 — a reserved seam with a registered diagnostic is a scope-lock violation (filed as `FD-12` in `technical_docs/IMPLEMENTATION.md` Appendix B). `options.cross_validate` rewritten from "Cross-validate F3 output against F1/F2" to a behavioural definition (emits `VW-14`/`VW-15`/`VW-16` warnings, never errors). Hand-edit-detection code `VI-03` killed per G3(c): replaced with a fixed CLI pre-flight warning printed every re-trim; architecture doc §1.2.8 Case 2, `ENGINEERING.md` §12 scenarios 16–17 and §16 Q2, and `DIAGNOSTIC_CODES.md` §1.3 all updated. **Doc-structure cleanup**: §1.5.4 stopped forward-referencing the never-authored `technical_docs/TECHNICAL_IMPLEMENTATION.md` and instead cites the three live peer documents (`ENGINEERING.md`, `technical_docs/IMPLEMENTATION.md` (parser section), `IMPLEMENTATION_ROADMAP.md`); `ENGINEERING.md` §1.8.5 retired-codes table replaced with a single sentence stating the registry is compact; §15 adoption roadmap collapsed to a pointer to `technical_docs/IMPLEMENTATION_ROADMAP.md`; §12 scenarios 30/31/32 dropped (Windows case-insensitive paths, symlinks, non-ASCII paths — absorbed into general `pathlib` behaviour, not worth a row each). Stray `json_kit/base-v1.schema.json`, `feature-v1.schema.json`, `project-v1.schema.json` duplicates at `json_kit/` root deleted (canonical location is `schemas/`). `FD-10 machine-readable-cli-output`, `FD-11 multi-platform-domain-support`, and `FD-12 template-script-generation` filed. `docs_old/` archive directory deleted. |
| 2026-04-22 | **Final handoff review fixes.** Fixed `schemas/feature-v1.schema.json` line 110: incorrect VW-10 reference changed to VW-18 (VW-10 is reserved, VW-19 is `cross-source-fe-vetoed`, and only VW-18 applies to `procedures.exclude` being vetoed). Unified empty-procs-array handling: `VE-03` now fires for both `procedures.include` and `procedures.exclude` (schema enforces `minItems: 1` on `procEntry.procs`); previous "silent no-op" text for exclude removed. Updated DIAGNOSTIC_CODES.md VE-03 description to reflect unified behavior. Created comprehensive FINAL_HANDOFF_REVIEW.md with devil's advocate analysis and sign-off for agent buildout. |
| 2026-04-24 | **F3 `options.generate_stack` + `json_kit/` dissolution.** Added `options.generate_stack` (boolean, default `false`) to the base JSON schema. When `true` with non-empty `stages`, the generator (P5b) emits one `<stage>.stack` per resolved stage alongside `<stage>.tcl` using the N/J/L/D/I/O/R format (see §3.6). D-line derivation: `dependencies` (one line each) > `load_from` (single line) > bare `D`. Generated `.stack` files register in `CompiledManifest.file_decisions` as `GENERATED` with reason `fi-literal`, participating in the trimmer skip-set and audit bundle exactly like `.tcl` run scripts. Resolved OQ-01 via this flag. Feature marked **newly implemented, untested against real customer domains** — companion agent memory updated to solicit feedback. Separately, the standalone `json_kit/` package was dissolved now that the runtime has shipped: `json_kit/schemas/` → `schemas/` at repo root, `json_kit/examples/` → `examples/`, `json_kit/docs/JSON_AUTHORING_GUIDE.md` → `technical_docs/JSON_AUTHORING_GUIDE.md`, `json_kit/validate_jsons.py` → `scripts/validate_jsons.py`, `json_kit/agent/DOMAIN_ANALYZER.md` → `.github/agents/domain-analyzer.agent.md`. Added `VERSION.txt` at repo root as a machine-readable release-number file paired with `VERSION.md`. All active-tense path references across CONTRIBUTING, README, `doc/*`, `.github/instructions/*`, the companion agent, the architecture doc (§3 canonical-JSON-reference callout), and code docstrings (`config/schema.py`, core model modules, `config/__init__.py`) were cascaded. Historical revision-log entries that mention `json_kit/` are left intact as point-in-time records. |
...
| 2026-04-24 | **0.3.2 — Companion consolidation + discoverability.** The former `.github/agents/domain-analyzer.agent.md` was absorbed into `.github/agents/chopper-agent.agent.md`, making the companion the **single user-facing agent** for anything Chopper-related. Companion card gained: Operating Modes (`analyze-only` vs `full-loop`), explicit Q1–Q5 Discovery Protocol, JSON Templates & Checklists (base/feature/project skeletons), Schema Error → Fix Mapping, Bootstrapping a New Domain playbook, Common CLI Workflows (Bisect / Compare-two-runs / Prove-JSON-safe / Explain-a-diagnostic), and a tier-2 greeting menu (Tier 1 "where are you starting from?" → Tier 2 full capability list). A prompt library was created at `.github/prompts/` with six ready-to-use starting points (`bootstrap-domain`, `explain-last-run`, `why-was-dropped`, `validate-my-jsons`, `bisect-feature-breakage`, `report-chopper-bug`). `doc/USER_MANUAL.md` cross-references the companion at the top of the Operating Tasks section. README `🤖 Two AI Companions` section rewritten as `🤖 Meet the Companion` (single agent); analyzer badge removed; the stale `.github/agents/domain-analyzer.agent.md` link in the 0.3.0 json_kit-dissolution history row unlinked with a note pointing to this absorption. No runtime, schema, diagnostic-registry, or scope-lock changes — agents, docs, and version files only. |

| 2026-04-24 | **0.4.0 — MCP stdio surface (read-only).** Narrowed the MCP row of the scope-lock (closed decision §1) from "all MCP integration closed" to "destructive MCP surface closed; stdio read-only `chopper mcp-serve` permitted". Added architecture doc §3.9 specifying the transport (stdio only, no TCP/HTTP/WebSocket), the three exposed tools (`chopper.validate`, `chopper.explain_diagnostic`, `chopper.read_audit`), and the destructive-tool guard (`chopper.trim` and `chopper.cleanup` are never registered). Added **FR-42** stating the contract. Added `PE-04 mcp-protocol-error` to the diagnostic registry (exit code 4, emitted only from `src/chopper/mcp/`). Added `mcp>=1.0,<2` as a **hard runtime dependency** in `pyproject.toml`. Created `src/chopper/mcp/` package (`server.py`, `tools.py`) and wired `mcp-serve` into `src/chopper/cli/` with lazy-import pattern so core CLI paths are unaffected by MCP-side changes. `chopper.read_audit` returns the **full** JSON contents of every file under a `.chopper/` bundle keyed by relative path (not a curated summary). CLI_REFERENCE, ENGINEERING §7, DIAGNOSTIC_CODES, and FUTURE_PLANNED_DEVELOPMENTS all cascaded. Companion card gained a "Calling Chopper from an MCP client" callout. Version bumped 0.3.3 → 0.4.0 (SemVer minor: new subcommand + new hard dep). |

| 2026-04-24 | **0.5.0 — Tool-command pool (TI-01).** Real EDA domains emit dozens to thousands of `TW-02 unresolved-proc-call` warnings per trim run, one per vendor-tool-command call (`get_app_var`, `set_dont_touch`, `report_timing`, …). The volume buries genuine `TW-02` hits on actually-missing procs. Added architecture doc **§3.10 Tool-Command Pool** and **FR-44** specifying a domain-agnostic registry of known external tool-command names, seeded 0.5.0 with PrimeTime (~850 commands shipped as `src/chopper/data/tool_commands/pt.commands`). Composition is the union of always-loaded built-in `.commands` files and user-supplied lists passed via the repeatable CLI flag `--tool-commands <path>`. Extended **§5.4 P4 trace** with an explicit six-step decision ladder pinning the pool check to the TW-02 branch only (never intercepts `TW-01` ambiguity, `TW-03` dynamic, or resolved tokens). Added new **`TI` diagnostic family** (Trace Info) to `DIAGNOSTIC_CODES.md` with `TI-01 known-tool-command` (severity info, exit 0, not counted against `--strict`). Matching rule: raw-token equality OR namespace-stripped leaf equality against pool entries (pool itself contains bare names only). **Deliberate non-features** (rejected during review): no base-JSON `options.tool_commands` field; no feature-JSON surface; no per-vendor opt-in; no effect on F1/F2/F3. Tool-command lists describe tools, not domains — they are infrastructure-owned and live outside the checked-in domain tree. Cascaded to `CLI_REFERENCE.md` (new `--tool-commands` flag on `validate` and `trim`), `LoadedConfig.tool_command_pool`, `TracerService` (new pool-check step before `_emit_tw02`), and a new fixture + integration test reproducing the STA-PT scenario from the user report. |
| 2026-04-25 | **Companion-assisted bug filing.** Added §5.11.7 specifying the presentation-layer workflow for the Chopper Agent: accept local evidence paths, package them into a non-`.chopper` zip convenience bundle, extract report facts from audit artifacts, render the GitHub issue-template body, and create the issue automatically when the host exposes an authenticated issue-creation transport such as `gh issue create`. The section also fixes the default fallback explicitly: if the transport is missing or fails, the same run must fall back to simple local behavior by keeping the rendered issue body and local bundle paths. Binary attachment upload remains outside the v1 contract, and the companion must not fake attachments by committing audit bundles into the repository or pushing them to ad-hoc external storage. Filed deferred `FD-13` for any future host-integrated attachment transport. |
| 2026-04-25 | **Tcl-parser fidelity uplift driven by sta_pt bug-report wave.** Five production bugs filed against the `sta_pt` domain were triaged, fixtured (verbatim from production source), and fixed: (1) `TW-02_quoted_string_semicolon_misparse` — the tokenizer treated `"` as literal inside brace-delimited proc bodies, so `puts "...;..."` produced a bogus command split and ~80 false `TW-02` per run. **Corrected:** the tokenizer now applies Tcl Endekas/Dodekalogue rule 5 — an unescaped `"` at a word boundary opens a quoted word at *any* brace depth, with a separate `quoted_bracket_depth` counter so inner `[...]` substitutions cannot prematurely close the outer word. Fixture `tests/fixtures/bug_reports/quoted_semicolon.tcl`. Pitfall **P-01** in `technical_docs/IMPLEMENTATION.md` (pitfalls) rewritten; `technical_docs/IMPLEMENTATION.md` (parser section) §1.3.0 transition table and §1.3.3 / §1.3.3.1 / §1.3.3.2 rewritten. (2) `TW-02_regex_literal_misparse` — `regexp` / `regsub` / `exec` / `glob` / `string match` brace arguments contain regex/glob/shell text but the call extractor walked them and emitted `TW-02` for `ERROR`, `Warning`, `L`, `o`, `g`, etc. **Corrected:** added a structural pre-pass in `call_extractor.py` that marks every `LBRACE…RBRACE` argument of an opaque command as `skip` for both cmd-position classification AND the `\[<name>` embedded-bracket regex. Recursion into code-block braces (`if`, `while`, `for`, `foreach`, `foreach_in_collection`, `catch`, `try`, `eval`, `uplevel`, `namespace eval`, `expr`) ensures opaque commands nested inside `if {[catch {…} ]}` are still reached. Fixture `regex_literals.tcl`. New pitfall **P-38**; new `technical_docs/IMPLEMENTATION.md` (parser section) §1.5.3.1 / §1.5.3.2. (3) `TW-02_switch_pattern_label_misparse` — `switch $x { a - b - c { … } }` extracted `a`, `b`, `c` as proc calls (32 false hits from a single production proc). **Corrected:** the same pre-pass detects `switch` invocations, locates the body brace, and marks every odd-indexed WORD inside it as `skip` (pattern labels and `-` fall-through markers are literals, not commands). Fixture `switch_patterns.tcl`. New pitfall **P-39**. (4) `PW-11_PI-04_dpa_line_continuation_misparse` — the previous regex-based DPA name extractor could not balance the nested `{…}` of `-define_args` and concatenated option-list fragments onto the proc name, firing false `PW-11` and a `PI-04` whose detail ended in a stray `\`. **Corrected:** `_extract_dpa_proc_name` now strips the keyword prefix and takes the first whitespace-delimited token only — independent of how many continuation lines or how nested the option-list braces are. The PI-04 message detail is rstrip'd of `\r\n` and trailing `\`. Fixture `dpa_multiline.tcl`. New pitfall **P-40**; `technical_docs/IMPLEMENTATION.md` (parser section) §1.4.6 DPA-name code block rewritten. (5) `diagnostics_file_null_for_p4_p6` — 617 of 621 entries in `.chopper/diagnostics.json` had `"file": null`, making editor jump-to-source useless. **Corrected:** `compiler/trace_service._emit_tw01/02/03/04` and `_emit_ti01` now pass `path=caller.source_file`; `validator/functions` `VW-05` / `VW-06` recover the path from `<file>::<qname>` via the new `_path_from_canonical` helper. New pitfall **P-41**; new architecture-doc invariant in §1.5.12.4. Regression tests added under `tests/unit/parser/test_bug_report_regressions.py`, `tests/unit/compiler/test_tracer.py`, `tests/unit/validator/test_validator.py`. Synthetic precursor tests for the same scenarios were retired. Final state: `_repro_check.py` reports zero false-positive call extractions; full unit suite at 836 passing (one pre-existing unrelated failure). No JSON schema, exit-code, or scope-lock changes — purely a fidelity uplift to the Tcl parser plus a serialisation-completeness fix. |

| 2026-04-25 | **0.5.0 — `chopper validate --features` accepts directory entries.** Real regression flows need to validate every feature JSON under `jsons/features/` in one shot. Added **FR-43** and extended bible §5.1: for `chopper validate` only, any entry in the `--features` comma-separated list may be a directory path; it expands in place to the sorted (lexicographic), non-recursive set of its immediate `*.json` children (POSIX-normalized). Files and directories may be mixed freely. `chopper trim` (and `--project` in any subcommand) are intentionally **unchanged** — they still require explicit per-file paths so the ordered feature sequence recorded in audit artifacts stays unambiguous; passing a directory to `trim` fails at config load. Implemented as a validate-only rewrite step (`_expand_feature_dirs`) in `src/chopper/cli/commands.py` before `_make_context`; no changes to compiler, trimmer, or audit surfaces. Cascaded to `CLI_REFERENCE.md`, `doc/USER_MANUAL.md`, `doc/IMPLEMENTATION_GUIDE.md`, and `README.md` (new Mode 2b example). Ships with seven new unit tests in `tests/unit/cli/test_commands.py` covering pass-through, directory expansion + sort order, mixed entries, empty dir, and the validate-expands / trim-does-not-expand contract. |
...
| 2026-05-01 | **0.8.1 — O1–O6 optimization wave + setup-script polish.** Performance uplift completing all six tracked optimizations. O1: `LoadedConfig.domain_file_cache` (built once by P1 during glob expansion) is now reused by P2's full-domain harvest phase to filter for `.tcl` files instead of re-walking the filesystem — eliminates a redundant full directory scan on every trim run. O2: compiler `merge_service` builds `short_to_canonical` once per `ParseResult` via `_build_short_to_canonical()` and threads it into both classify and aggregate passes, cutting rebuilds from `2 × S × F` to `S`. O3/O4: audited and confirmed no-op (see IMPROVEMENTS.md). O5/O6: shipped in 0.8.0. Setup scripts hardened: proxy environment variables (`HTTP_PROXY`/`HTTPS_PROXY`/`http_proxy`/`https_proxy`) are now exported before the step-[1/6] `git pull` so all network operations run through the proxy from the start; the step-[5/6] chopper uninstall is now conditional (`pip show chopper` guard before `pip uninstall`) so fresh-venv installs do not log a spurious "not installed" error. No schema, diagnostic-registry, CLI surface, or behavioral changes. |
...
| 2026-05-06 | **0.9.1 — Documentation restructure + agent consolidation + conversational owner upgrade.** Rolled up an 8-stage documentation refactor and a `.github/` agent rationalization into a single user-visible release. **Doc restructure (stages 1–8):** renamed `CLI_HELP_TEXT_REFERENCE.md` → `CLI_REFERENCE.md`; consolidated `TCL_PARSER_SPEC.md`, `RISKS_AND_PITFALLS.md`, `IMPLEMENTATION_DECISION_LOG.md`, and `FUTURE_PLANNED_DEVELOPMENTS.md` into `IMPLEMENTATION.md` (parser §1, pitfalls §2–3, Appendix B `FD-xx` register); renamed `ARCHITECTURE_PLAN.md` → `ENGINEERING.md` and aligned its scope-lock with this doc; renamed `chopper_description.md` → `ARCHITECTURE.md` (this file) so the architecture doc has the obvious filename; cross-reference verification pass cleaned every stale citation across `src/`, `doc/`, `examples/`, `schemas/`, `tests/`, and `.github/`; semantic doc-vs-code validation fixed four behavioral drifts (MCP §3.9 wording, audit-bundle table, module table, fixture-catalog references). **Agent consolidation:** deleted four redundant `.github/agents/*.agent.md` files (`devils-advocate`, `principal-software-engineer`, `swe-subagent`, `Thinking-Beast-Mode`) and absorbed their behaviors into the surviving agents as internalized personas; renamed `chopper-domain-companion.agent.md` → `chopper-agent.agent.md` so the single user-facing agent is just "Chopper Agent"; added a System Check section to `.github/instructions/project.instructions.md` and to every active agent so they detect tcsh (Unix primary) / PowerShell (Windows secondary) / cmd.exe / bash-zsh (fallback) before issuing shell commands. **Conversational owner upgrade:** rewrote `chopper-agent.agent.md` as a software-dev owner with a Documentation Index (every authoritative doc by purpose), an In-Situ Bug Fixes section (always-safe / ask-before-fixing / hard-rules), a tier-3 "Software-dev-owner mode" entry on the greeting menu, and an explicit conversational-style rule (every turn ends with one focused next question and 2–3 active suggestions). No schema, diagnostic-registry, CLI surface, exit-code, runtime, or pipeline-phase changes; this is a pure tooling-and-docs release. Version bumped 0.9.0 → 0.9.1. |
| 2026-05-07 | **1.0.0 — Coverage hardened to ≥98% across all source files.** Added 74 surgical unit tests in `tests/unit/test_coverage_98.py` exercising previously-uncovered defensive branches across `validator/functions.py`, `parser/service.py`, `parser/tokenizer.py`, `compiler/trace_service.py`, `compiler/merge_service.py`, `compiler/flow_resolver.py`, `config/service.py`, `audit/service.py`, `audit/sloc.py`, `adapters/fs_memory.py`, `trimmer/indentation.py`, `trimmer/proc_dropper.py`, `cli/main.py`, `cli/commands.py`, and `core/models_audit.py`. Total line+branch coverage: 92% → **98.75%**. Test suite: 1005 → **1079 passing**, 7 skipped. No production code drift; no schema, diagnostic-registry, CLI surface, exit-code, runtime, or pipeline-phase changes. Version bumped 0.9.2 → 1.0.0 to mark the v1 stability milestone now that all six pipeline phases ship at near-complete coverage. |
| 2026-05-06 | **0.9.2 — End-user documentation consolidated and renamed to `user_docs/`.** Deleted the four-guide `doc/` folder (`README.md`, `USER_MANUAL.md`, `BEHAVIOR_GUIDE.md`, `TECHNICAL_GUIDE.md`, `IMPLEMENTATION_GUIDE.md`) and replaced it with a leaner three-part suite under `user_docs/`: **`01_OVERVIEW.md`** (problem → solution → F1/F2/F3 → JSON structures → optional switches → BKMs → caveats → ownership → example map), **`02_CLI_GUIDE.md`** (every subcommand, every flag, three invocation modes, dry-run vs validate, live-trim filesystem semantics, `.chopper/` bundle, deep examples for first-time / iteration / bisect / CI / tool-command / stage-generation, troubleshooting, bug-reporting), and **`03_HOW_CHOPPER_WORKS.md`** (where to use vs not, P0–P7 pipeline, the three governing rules with worked example, determinism contract, trace-as-X-ray, performance envelope, error model, module map, ports surface, FAQ). The new structure follows a thesis-style abstraction designed to ramp a reader up from the onboarding presentation in 60–90 minutes without fatigue. The Chopper-vs-Flow-Builder framing is now stated explicitly in the overview's opening so the "Chopper" name does not mislead — both tools derive specialised flows from a fully feature-rich domain; the distinction is that Flow Builder targets file-based flows while Chopper supports both file-based and proc-based flows. **All back-references cascaded** in the same commit: `README.md` documentation table, `CONTRIBUTING.md` "Before You Start" table, `.github/agents/chopper-agent.agent.md` Documentation Index (three new `user_docs/*` rows replace the four `doc/*` rows), and `.github/ISSUE_TEMPLATE/config.yml` "Question or usage help" link all point at `user_docs/`. Historical revision-history entries that mention the old `doc/` paths are intentionally left intact as point-in-time records. No schema, diagnostic-registry, CLI surface, exit-code, runtime, or pipeline-phase changes — pure docs release. Version bumped 0.9.1 → 0.9.2. |
| 2026-05-07 | **1.1.0 — `--domain` priority + selected-JSON preservation in rebuilt domain.** Two coordinated changes. **(1) `--domain` priority over cwd (§5.1).** Domain-root resolution is now an explicit priority list: `--domain` (highest) → backup-cwd suffix-strip guard → `Path.cwd()`. The previous "if `--domain` is provided alongside `--project`, it must resolve to `Path.cwd()`" exit-2 gate is removed — `--domain` is the authoritative source of truth, not a constraint to validate against cwd. `VE-17 project-domain-mismatch` now compares `domain_root.name.casefold()` (resolved per the priority list) against `project.domain.casefold()` rather than `Path.cwd().name`. The validator already read `ctx.config.domain_root.name`, so this was a spec-vs-source alignment, not a behavior change in the validator. **(2) JSON preservation in rebuilt domain (§5.6).** After P5 per-file dispatch succeeds on a live trim, the trimmer copies the selected JSON inputs into the rebuilt `<domain>/jsons/`. In-tree inputs (already under the domain root) are preserved at their original relative path; out-of-tree inputs land in `<domain>/jsons/_external/<NN>_<basename>` with a numeric prefix mirroring the audit-bundle convention. Preservation is unconditional, runs only on live trim, and emits `VW-20 audit-write-failed` on I/O errors. **(Later revised in 2.10.0 to copy the entire jsons/ tree; see that entry.)** Cascaded to `CLI_REFERENCE.md` (`--domain` description), `IMPLEMENTATION.md` (P-25 phrasing), `DIAGNOSTIC_CODES.md` (VE-17 description), and `tests/SPEC_COVERAGE_AUDIT.md` (drift row → covered). New trimmer helper `preserve_input_sources` in `src/chopper/trimmer/input_preserver.py` (invoked from the runner at the P5a tail); new `TrimReport.inputs_preserved: int` field. Version bumped 1.0.0 → 1.1.0. |
| 2026-05-07 | **1.2.5 — Validation-pass version bump.** Patch-level bump after the post-1.2.0 doc cross-validation pass. Two prose drifts repaired in place: the 1.1.0 row in this revision history referenced a `_copy_input_sources` helper in `trimmer/service.py`; the live implementation is `preserve_input_sources` in [src/chopper/trimmer/input_preserver.py](../src/chopper/trimmer/input_preserver.py) (called from the runner at the P5a tail). The README 1.1.0 entry pointed at §5.5 for preserved inputs; the subsection lives at §5.6. No source, schema, diagnostic-registry, CLI surface, exit-code, runtime, or pipeline-phase changes. Version bumped 1.2.0 → 1.2.5. |
| 2026-05-07 | **1.2.0 — Conditional `_backup` redirect + `VI-03 domain-suffix-strip-applied`.** The two-tier domain-root resolution from 1.1.0 (`--domain` priority + cwd-only backup-strip guard) is replaced by a single rule: pick the candidate (from `--domain` if present, else cwd), then if its basename ends in `_backup` **and** the stripped sibling exists as a directory, redirect to that sibling. The redirect now applies to `--domain` as well, not only to cwd. Rationale (§5.1 "Operational target vs source of truth"): when a live `<domain>/` exists alongside `<domain>_backup/`, the live path is the operational target and the backup is the source of truth — pointing `--domain` (or cwd) at the backup was previously valid-but-broken (it produced a sibling `<domain>_backup_backup/` and severed the link to the real domain). The redirect protects two failure modes — cwd-accident and flag-confusion — that share the same corrective action. **Conditional, not unconditional:** if the stripped sibling does **not** exist on disk, the candidate is honored as-is, so a coincidentally `_backup`-suffixed domain remains usable. The redirect is single-shot: `foo_backup_backup` redirects to `foo_backup` only when `foo_backup/` exists, never recursively to `foo`. When the redirect fires, Chopper emits `VI-03 domain-suffix-strip-applied` (info severity, exit 0; never escalated by `--strict`) so the resolution is visible in stderr and recorded in the audit bundle. Cascaded to `CLI_REFERENCE.md` (`--domain` description), `DIAGNOSTIC_CODES.md` (new `VI-03`), and `user_docs/02_CLI_GUIDE.md` (BKM and pitfall sections). Source change in [src/chopper/cli/commands.py](../src/chopper/cli/commands.py) `_resolve_domain_root` (now returns `(Path, Path \| None)` and the caller emits `VI-03` via the diagnostic sink); CLI test suite updated to flip the cwd-only premise and assert the conditional rule. Version bumped 1.1.0 → 1.2.0. |
| 2026-05-07 | **1.2.6 — Dual-mode launcher (installed console script + `python -m chopper`).** Pure developer-experience release. Added [src/chopper/__main__.py](../src/chopper/__main__.py) so `python -m chopper ...` resolves through `chopper.cli.main:main` exactly like the installed `chopper` console script. All four setup scripts ([setup.csh](../setup.csh), [setup.sh](../setup.sh), [setup.ps1](../setup.ps1), [setup.bat](../setup.bat)) now (a) prepend `<repo>/src` to `PYTHONPATH` idempotently so `python -m chopper` works from a fresh clone before any `pip install` regenerates the console-script shim, and (b) validate **both** `chopper --help` and `python -m chopper --help` in step `[6/6]`, printing one of three states: both OK, only the module form OK (warn + repair hint, do not abort), or both fail (abort). The existing `pip install -e .` step is retained, so `git pull` continues to pick up source changes immediately for both invocation forms. Rationale: when an editable install gets out of sync (stale shim, partial reinstall) the module form is the failsafe path that always reads the working tree. No source-package, schema, diagnostic-registry, CLI surface, exit-code, runtime, or pipeline-phase changes. Version bumped 1.2.5 → 1.2.6. |
| 2026-05-07 | **1.2.7 — Idempotent setup + de-hardcoded TFM bundle.** Pure setup / packaging release. **(1) Idempotent setup scripts.** All four scripts ([setup.csh](../setup.csh), [setup.sh](../setup.sh), [setup.ps1](../setup.ps1), [setup.bat](../setup.bat)) reuse a healthy existing `.venv` instead of recreating it on every invocation; opt out with `CHOPPER_FRESH=1`. The redundant `pip install -e . --force-reinstall --no-deps` step is removed so subsequent runs short-circuit when nothing changed (was triggering full dependency re-downloads). Added must-be-sourced guards (`if (! $?prompt)` for tcsh, `(return 0 2>/dev/null)` for bash, `$MyInvocation.InvocationName -ne '.'` for PowerShell) so executing instead of sourcing fails fast with a remediation hint. **(2) Three-tier Python resolution strategy** applied uniformly: PATH (`python3.13` / `python3` / `python` requiring `sys.version_info >= (3, 13)`) → Intel EC `/usr/intel/bin/python3.13.2` (Unix) → local `uv python install 3.13.4` into `<repo>/.local-python/3.13/` if `uv` is available, or winget-installed Python 3.13 on Windows. Replaces the prior strict pin to a single EC interpreter. **(3) TFM bundle de-hardcoded.** [Makefile](../Makefile) `bundle` target replaces `EC_PYTHON := /usr/intel/bin/python3.13.2` with `BUNDLE_PYTHON ?= python` so `vendor/` is populated using whichever Python is active in the dev's env (typically the `.venv` resolved by `setup.csh`); guard aborts the build if `BUNDLE_PYTHON` is `< 3.13`. The shipped launcher [scripts/dist/chopper.launcher.csh](../scripts/dist/chopper.launcher.csh) now resolves Python at deploy time via the same PATH-first → EC-fallback strategy, instead of execing a baked-in `/usr/intel/bin/python3.13.2`. Manifest field renamed `ec_python:` → `build_python:`. No source-package, schema, diagnostic-registry, CLI surface, exit-code, runtime, or pipeline-phase changes. Version bumped 1.2.6 → 1.2.7. |
