...
After P5 per-file dispatch succeeds and **only on a live (non-dry-run) trim**, the trimmer copies every JSON listed in `compiled_manifest.input_sources` into the rebuilt `<domain>/jsons/`. This makes the rebuilt domain self-contained: a downstream consumer can run `chopper validate --domain <domain> --base jsons/base.json [--features jsons/features/...]` and replay the exact selection that produced the rebuild, without needing the original infrastructure paths.

Resolution rules:

1. **In-tree input** — the JSON's absolute path is under the domain root. The trimmer preserves it at its original domain-relative path (typically `<domain>/jsons/base.json` and `<domain>/jsons/features/<name>.feature.json`). Because P5 has already torn down the live domain by the time this step runs, the in-tree copy is read from the backup tree (`<domain>_backup/jsons/...`), not re-read from disk under the rebuilt domain.
2. **Out-of-tree input** — the JSON's absolute path is anywhere else (a wrapper script's shared config, a CI infrastructure path, or a project JSON authored alongside its repo). The trimmer copies the file to `<domain>/jsons/_external/<NN>_<basename>` where `<NN>` is a two-digit zero-padded sequence number derived from `compiled_manifest.input_sources` ordering. The numeric prefix mirrors the audit-bundle convention under `.chopper/inputs/` and prevents name collisions when two external inputs share a basename.
...
- Preservation is **unconditional** — selected inputs survive regardless of whether they are listed in any `files.include` / `files.exclude` clause. Chopper owns this behavior; user JSONs cannot opt out.
- **Only selected inputs survive.** Unselected feature JSONs that happen to live under `<domain>/jsons/features/` are not preserved (they would be neither in `input_sources` nor in `files.include`). Domain owners who keep a curated `<domain>/jsons/` checked into version control should treat the rebuilt domain as a derived artifact, not a working tree for authoring.
...
| 2026-05-07 | **1.1.0 — `--domain` priority + selected-JSON preservation in rebuilt domain.** Two coordinated changes. **(1) `--domain` priority over cwd (§5.1).** Domain-root resolution is now an explicit priority list: `--domain` (highest) → backup-cwd suffix-strip guard → `Path.cwd()`. The previous "if `--domain` is provided alongside `--project`, it must resolve to `Path.cwd()`" exit-2 gate is removed — `--domain` is the authoritative source of truth, not a constraint to validate against cwd. `VE-17 project-domain-mismatch` now compares `domain_root.name.casefold()` (resolved per the priority list) against `project.domain.casefold()` rather than `Path.cwd().name`. The validator already read `ctx.config.domain_root.name`, so this was a spec-vs-source alignment, not a behavior change in the validator. **(2) Selected-JSON preservation in rebuilt domain (§5.6).** After P5 per-file dispatch succeeds on a live trim, the trimmer copies every JSON in `compiled_manifest.input_sources` into the rebuilt `<domain>/jsons/`. In-tree inputs (already under the domain root) are preserved at their original relative path; out-of-tree inputs land in `<domain>/jsons/_external/<NN>_<basename>` with a numeric prefix mirroring the audit-bundle convention. Preservation is unconditional (does not require a `files.include` entry), runs only on live trim (skipped under `--dry-run`), and emits `VW-20 audit-write-failed` on I/O errors so the run continues. Only **selected** inputs survive — unselected feature JSONs that happen to live under `<domain>/jsons/features/` are not preserved, because the rebuilt domain mirrors the current run, not the working tree. Cascaded to `CLI_REFERENCE.md` (`--domain` description), `IMPLEMENTATION.md` (P-25 phrasing), `DIAGNOSTIC_CODES.md` (VE-17 description), and `tests/SPEC_COVERAGE_AUDIT.md` (drift row → covered). New trimmer helper `preserve_input_sources` in `src/chopper/trimmer/input_preserver.py` (invoked from the runner at the P5a tail); new `TrimReport.inputs_preserved: int` field. Version bumped 1.0.0 → 1.1.0. |
