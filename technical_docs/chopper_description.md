...
This architecture document defines what Chopper must do. How the implementation structures and preserves those contracts is split across three peer documents: [`technical_docs/ARCHITECTURE_PLAN.md`](ARCHITECTURE_PLAN.md) (ports, services, stage layering), [`technical_docs/IMPLEMENTATION.md` (parser section)](IMPLEMENTATION.md) (P2 engineering baseline), and [`technical_docs/IMPLEMENTATION_ROADMAP.md`](IMPLEMENTATION_ROADMAP.md) (stage gates, DoD, checkpoints).
...
The repository-scoped Chopper Domain Companion and its prompt library are presentation-layer workflows that sit **above** the engine. They may automate bug-report preparation and issue filing without changing the eight-phase runtime or the audit-artifact contract.
...
- The repository layout is defined in [`technical_docs/ARCHITECTURE_PLAN.md`](ARCHITECTURE_PLAN.md) §3. Every source module lives under `src/chopper/`. No sibling-module imports across service packages — services depend only on `core/` and their own submodules. Cross-cutting data lives in phase-owned `core/models_*.py` modules that callers import directly; cross-cutting protocols live in `core/protocols.py`.
